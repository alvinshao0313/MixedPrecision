...
import os
import logging
import model.deepseek.modeling_deepseek as deepseek
# import model.mixtral.modeling_mixtral as mixtral
...
DEEPSEEK_MODEL = deepseek.DeepseekForCausalLM
DEEPSEEK_LAYER = deepseek.DeepseekDecoderLayer
...
def get_deepseek(model_name):
    model = deepseek.DeepseekForCausalLM.from_pretrained(model_name, torch_dtype='auto',
                                                         low_cpu_mem_usage=True)
    model.seqlen = 2048
    logging.info('---> Loading {} Model with seq_len: {}'.format(model_name, model.seqlen))
    return model
...
def get_model(
    ...
    if 'llama' in model_name:
        ...
        return get_deepseek(model_name)
    else:
...
def get_pre_head_layernorm(model, model_type):
    if model_type == LLAMA_MODEL:
        ...
        pre_head_layernorm = model.model.norm
        assert isinstance(pre_head_layernorm, deepseek.DeepseekRMSNorm)
    else:
        raise ValueError(f'Unknown model type {model_type}')
